...
export class PlayQueueTableRowStore implements IPlayQueueTableRowStore {
	static nextId = 1;
	...
	constructor(
		readonly playQueue: IPlayQueueTableStore,
		readonly dto: PlayQueueItemDto,
	) {
		this.id = PlayQueueTableRowStore.nextId++;
	...
	): IPlayQueueTableRowStore {
		return new PlayQueueTableRowStore(playQueue, dto);
	}
