...
public class GetPlaylistEndpoint(ISender sender) : ControllerBase
{
	[Tags("Media Player - Playlists")]
	[HttpGet("/media-player/playlists/{id}")]
	[Produces<GetPlaylistResponseDto>]
	public Task<IResult> HandleAsync(Guid id, CancellationToken ct)
	{
		return sender.Send(new GetPlaylistQuery(Id: id), ct).ToMinimalApiResult();
	}
}
